...
Example of use :
python Model_training_full.py -m myModel1 -a -r test
"""
import numpy as np
import argparse
import os
...
def main():
    ...
    print('Loading data')
    data, labels, indexes, ratio = load_data_multi_species(species_list, window, args.step)

    np.random.shuffle(indexes)
    train_indexes = indexes[int(len(indexes)*args.validation):]
    val_indexes = indexes[:int(len(indexes)*args.validation)]
